...
function Projects() {
    ...
    const listProjectsCadeiras = projects.map((project) =>
        <div style={{listStyle: 'none', paddingLeft: 0, paddingRight: 30, color: "black", fontWeight: "bold", width: "auto", margin:"auto",  fontStyle: "italic"}}>
            {project.cadeira}</div>);
    const listProjectsProjects = projects.map((project) =>
        <div style={{listStyle: 'none', paddingRight: 90, color: "black", fontWeight: "bold", width: "auto", margin:"auto",  fontStyle: "italic"}}>
            {project.projeto}</div>);
    const listProjectsTeams = projects.map((project) =>
        <div style={{listStyle: 'none', paddingRight: 90, color: "black", fontWeight: "bold", width: "auto", margin:"auto",  fontStyle: "italic"}}>
            {project.equipa}</div>);
    const listProjectsTime = projects.map((project) =>
        <div style={{display: "block", paddingLeft: 40, width: "auto", margin:"auto", gridTemplateColumns: "repeat(2, 1fr)"}}>
            <div className="timer" style={{ width: "auto", margin:"auto"}}>{project.time}</div>
